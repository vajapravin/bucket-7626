...
function initTransactionsApi(server) {
    server.route({
        ...
        handler: req => {
            ...
            return get_transaction_1.getTransaction(transactionId, traceId, setup).catch(defaultErrorHandler);
        }
    });
    ...
    server.route({
        ...
        handler: req => {
            const { transactionId } = req.params;
            const setup = setup_request_1.setupRequest(req);
            return get_spans_1.getSpans(transactionId, setup).catch(defaultErrorHandler);
        }
    });
}
